...
N_DISCRETE_VELOCITIES = 21


LATTICE_INDICES = jnp.arange(N_DISCRETE_VELOCITIES)
...
print("Lattice velocity shape ", LATTICE_VELOCITIES.shape)
print(LATTICE_INDICES)
OPPOSITE_LATTICE_INDICES = jnp.array([
    0, 3, 4, 1, 2, 7, 8, 5, 6,11,12,9,10,15,16,13,14,19,20,17,18
])

# https://arxiv.org/pdf/0908.4520.pdf Table 1
LATTICE_WEIGHTS = jnp.array([91/324]+
                            4*[1/12]+
                            4*[2/27]+
                            4*[7/360]+
                            4*[1/432]+
...
RIGHT_VELOCITIES = jnp.array([1, 5, 8,9,13,16,17])
UP_VELOCITIES = jnp.array([2, 5, 6,10,18])
LEFT_VELOCITIES = jnp.array([3, 6, 7,11,14,15,19])
DOWN_VELOCITIES = jnp.array([4, 7, 8,12,15,16])
PURE_VERTICAL_VELOCITIES = jnp.array([0, 2, 4,10,12,18,20])
PURE_HORIZONTAL_VELOCITIES = jnp.array([0, 1, 3,9,11,13])
...
def get_macroscopic_velocities(discrete_velocities, density):
    # print(f"shape of discrete velocities: {discrete_velocities.shape}")
...
def main():
    ...
    print(f"relaxation_omega: {relaxation_omega}")
    # Obstacle Mask: An array of the shape like X or Y, but contains True if the point belongs to the obstacle and False if not
    obstacle_mask = (
        jnp.sqrt(
            (
                X
    ...
    def update(discrete_velocities_prev):
        ...
        macroscopic_velocities_prev =\
            macroscopic_velocities_prev.at[0, 6:-6, :].set(
                velocity_profile[0, 6:-6, :]
            )
    ...
    for iteration_index in tqdm(range(N_ITERATIONS)):
        ...
        if iteration_index % PLOT_EVERY_N_STEPS == 0 and VISUALIZE and iteration_index > SKIP_FIRST_N_ITERATIONS:
            ...
            fig.savefig(f"figures/D2Q21_jax/VonKarmon_D2Q21_jax_{iteration_index}.png")
            plt.close()


if __name__ == "__main__":
    import os
    if not os.path.exists("figures/D2Q21_jax"):
        os.makedirs("figures/D2Q21_jax")
    main()
